    //
    // Created by Shaked Pollak on 11/7/2025.
    //

    #include "../include/dns.h"

    #include <stdio.h>
    #define _CRT_RAND_S // for rand_s
    #include <stdlib.h>
    #include <string.h>
    #include <winsock2.h>

    uint16_t randomID() {
        // rand_s takes unsigned int = 32 bits in x64, but we want a 16 bit random ID
        unsigned int randomID;
        rand_s(&randomID); // safe for threads
        return (uint16_t)randomID; // takes the 16 MSB
    }

    struct dnsHeader buildHeader() {
        /* I'm building a dns query packet
         * So the flags are:
         *    QR = 0 -> Query/Response -> 0 for query, 1 for response - 1 bit
         *    OpCode = 0 -> 0 for standard domain name to IP query - 4 bit
         *    AA = 0 -> Authoritative Answer -> indicates if the response was from an authoritative server (set in the response by the server) - 1 bit
         *    TC = 0 -> Truncation -> indicates if the response is too large for the udp packet (set in the response by the server) - 1 bit
         *    RD = 1 -> Recursion Desired -> set to 1 if the client asks for recursion (set in the query by the client) - 1 bit
         *    RA = 0 -> Recursion Available -> set to 1 if the server supports recursion (set in the response by the server) - 1 bit
         *    Z = 0 -> bits for future use (for new protocol features) - 3 bits
         *    RCODE = 0 -> Response Code (set in the response by the server) - 4 bits
         */

        uint16_t flags = 0;  // sets the 8th bit to 1, the rest are 0
        flags |= (1 << 7); // sets the 8th bit to 1, the rest are 0
        const struct dnsHeader header = {.id = htons(randomID()), .flags = htons(flags), .QDcount = htons(1), .ANcount = htons(0), .NScount = htons(0), .ARcount = htons(0)};
        return header;
    }

    /* This function allocates memory to qname and dnsNameCpy
     * it frees dnsNameCpy inside the function
     * must free qname in main.c
     */
    struct Qname buildQname(const char* dnsName) {
        char* dnsNameCpy = malloc(strlen(dnsName)+1); // plus 1 for the null terminator
        strcpy_s(dnsNameCpy, strlen(dnsName)+1, dnsName);

        size_t totalSize = 1; // init with null terminator
        size_t offset = 0;
        char* qname = malloc(totalSize);
        qname[0] = 0x00;

        char* currentLabel = strtok(dnsNameCpy, ".");

        while (currentLabel != NULL) {
            size_t currentLabelLen = strlen(currentLabel);
            totalSize += (currentLabelLen + 1);
            char* temp = realloc(qname, totalSize); // qname is released (temp points to qname)

            if (temp == NULL) {
                printf("[ERROR] -- Couldn't reallocate memory, exiting...\n");
                free(qname);
                free(dnsNameCpy);
                exit(EXIT_FAILURE);
            }
            qname = temp;

            qname[offset] = (uint8_t)currentLabelLen;
            memcpy(qname + offset + 1, currentLabel, currentLabelLen);
            offset += (currentLabelLen + 1);

            currentLabel = strtok(NULL, "."); // strtok replaces the delim of the original string to the null terminator
        }
        qname[offset] = 0x00; // ends the qname with the null terminator
        totalSize = offset + 1;

        free(dnsNameCpy);

        struct Qname q = {qname, totalSize};
        return q;
        // qname will look like 7example3com0
    }

    struct dnsQuery buildQuery(const char* dnsName, int protocol) {
        /*
         *  Qname - Query Name
         *  Qtype - Type Of Query
         *  Qclass - Protocol Class of dns query
         *
         *  for domain name, Qtype = CNAME = 0x0005
         *  the standard Qclass for A (and AAAA) is the Internet Class - IN. Qclass = IN = 0x0001
         */

        uint16_t qtype = 0;

        if (protocol == IPv4OPTION)
            qtype = IPv4TYPE;
        else if (protocol == IPv6OPTION)
            qtype = IPv6TYPE;

        const uint16_t INClass = 0x01;


        const struct Qname qname = buildQname(dnsName);
        struct dnsQuery query = {.Qname = qname, .Qtype = htons(qtype), .Qclass = htons(INClass)};
        return query;
    }